...
type Provider interface {
	...
	GenerateTokens(userID string) (entity.TokenPair, error)
}
...
func (p *tokenProvider) GenerateTokens(userID string) (entity.TokenPair, error) {
	accessToken, err := p.generateAccessToken(userID)
	if err != nil {
		return entity.TokenPair{}, errwrap.Wrap("generate access token", err)
	}

	refreshToken, err := p.generateRefreshToken(userID)
	if err != nil {
		return entity.TokenPair{}, errwrap.Wrap("generate refresh token", err)
	}
